...
class _PageAccountDetailBlocState extends State<PageAccountDetailBloc> {
  ...
  Widget build(BuildContext context) {
    return BaseView<AccountsModel>(
      onModelReady: (model) => model.fetchCurrentAccount(),
            ...
            : Scaffold(
                ...
                appBar: AppBar(
                  actions: <Widget>[
                    FlatButton(
                      onPressed: () {
                        String instanceID = model.currentInstanceID;
                        if (instanceID.isNotEmpty) {
                          ...
                          print(
                              "No instanceID retrieved from successful login");
                        }
                      },
                ...
                body: SafeArea(
                  ...
                  children: <Widget>[
                    ...
                    buildTextfieldRow(
                        "AccountNameKey", "Account Name", widget.account.name),
                    buildTextfieldRow("IndustryTypeKey", "Industry Type",
                        widget.account.industryType),
                    buildTextfieldRow(
                        "StatusKey", "Status", widget.account.statusName),
                    buildTextfieldRow("ContactNumberKey", "Contact Number",
                        widget.account.contactPhone),
                    buildTextfieldRow("ContactEmailKey", "Contact Email",
                        widget.account.contactEmail),
                  ],
                )));
      },
    );
